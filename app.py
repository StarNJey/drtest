...
class WebCrawler:
    ...
    def search_and_crawl(self, query: str, max_results: int = 10) -> List[WebDocument]:
        """검색 + 크롤링 통합 메서드"""
...
최소 3개, 최대 10개의 검색 쿼리를 생성하세요."""
    ...
    def _parse_search_queries(self, response: str, state: ResearchState) -> List[SearchQuery]:
        ...
        for line in lines:
            line = line.strip()
            # 검색어
            if re.match(r"^(\d+\.\s*|-\s*)검색어:", line):
                text = re.sub(r"^(\d+\.\s*|-\s*)검색어:", "", line).strip()
                current_query["text"] = text
            # 우선순위
            elif re.match(r"^(\d+\.\s*|-\s*)우선순위:", line):
                nums = re.findall(r"\d+", line)
                priority = float(nums[0]) / 10.0 if nums else 0.5
                current_query["priority"] = priority
            # 카테고리
            elif re.match(r"^(\d+\.\s*|-\s*)카테고리:", line):
                category = re.sub(r"^(\d+\.\s*|-\s*)카테고리:", "", line).strip()
                current_query["category"] = category
            # 이유
            elif re.match(r"^(\d+\.\s*|-\s*)이유:", line):
                reason = re.sub(r"^(\d+\.\s*|-\s*)이유:", "", line).strip()
                current_query["reason"] = reason
            # 웹검색 여부
            elif re.match(r"^(\d+\.\s*|-\s*)웹검색:", line):
                flag = re.sub(r"^(\d+\.\s*|-\s*)웹검색:", "", line).strip().upper()
                current_query["search_web"] = "YES" in flag
    
            # 한 쿼리 완성 시 추가
            if all(key in current_query for key in ["text", "priority", "category", "reason"]):
                if "search_web" not in current_query:
                    current_query["search_web"] = True
                queries.append(SearchQuery(**current_query))  # type: ignore
                current_query = {}
...
부족한 정보나 추가 조사가 필요한 영역을 최대 5개까지 나열하세요."""
...
class AnalyzerAgent:
    ...
    def cross_validate_documents(self, query: str, state: ResearchState) -> Dict[str, Any]:
        """PDF와 웹 문서 간 교차 검증"""
        if not state.retrieved_docs and not state.web_docs:
            return {"consistency": 0.0, "conflicts": [], "consensus": [], "reliability": 0.0}
        
        # PDF와 웹 문서 분리
        pdf_docs = [doc for doc in state.retrieved_docs if doc.get('source_type') == 'PDF']
        web_docs = [doc for doc in state.retrieved_docs if doc.get('source_type') == 'WEB'] + state.web_docs
        
        # 교차 검증 수행
        conflicts = self._identify_conflicts(pdf_docs, web_docs, query)
        consensus = self._find_consensus(pdf_docs, web_docs, query)
        consistency = self._calculate_consistency(pdf_docs, web_docs, conflicts, consensus)
        reliability = self._assess_reliability(pdf_docs, web_docs)
    ...
    def _identify_conflicts(self, pdf_docs: List[Dict], web_docs: List[Dict], query: str) -> List[Dict]:
        """상충되는 정보 식별"""
        if not pdf_docs or not web_docs:
            return []
        
        # 샘플링을 통한 효율적 분석 (최대 4개씩)
        pdf_sample = pdf_docs[:4]
        web_sample = web_docs[:4]
        
        conflicts = []
        
        conflict_prompt = f"""다음 질문에 대해 PDF 문서와 웹 문서의 정보를 비교하여 상충되는 내용을 찾아주세요:

질문: {query}

PDF 문서 정보:
{chr(10).join([f"- {doc['text'][:200]}..." for doc in pdf_sample])}

웹 문서 정보:
{chr(10).join([self._get_web_text(doc)[:200] + "..." for doc in web_sample])}

상충되는 정보가 있다면 구체적으로 나열해주세요. 없다면 '상충 없음'이라고 답하세요."""

        messages = [
            {"role": "system", "content": "당신은 정보의 상충점을 분석하는 전문가입니다."},
            {"role": "user", "content": conflict_prompt}
        ]
        
        try:
            response = self._generate_llm_response(messages, max_tokens=500)
            if response and "상충 없음" not in response:
                conflicts.append({
                    "type": "PDF vs WEB",
            ...
            st.warning(f"상충 분석 실패: {e}")
        
        return conflicts
    
    def _find_consensus(self, pdf_docs: List[Dict], web_docs: List[Dict], query: str) -> List[str]:
        """공통된 정보 찾기"""
        if not pdf_docs or not web_docs:
            return []
        
        pdf_sample = pdf_docs[:3]
        web_sample = web_docs[:3]
        
        consensus_prompt = f"""다음 질문에 대해 PDF 문서와 웹 문서에서 공통으로 확인되는 정보를 찾아주세요:
...
공통된 핵심 정보만 간단히 나열해주세요."""

        messages = [
            {"role": "system", "content": "당신은 정보의 공통점을 찾는 전문가입니다."},
        ...
        try:
            ...
            st.warning(f"합의점 분석 실패: {e}")
        
        return []
    
    def _calculate_consistency(self, pdf_docs: List[Dict], web_docs: List[Dict], 
                             conflicts: List[Dict], consensus: List[str]) -> float:
        """일관성 점수 계산"""
        if not pdf_docs and not web_docs:
            return 0.0
        
        # 기본 일관성 (문서 존재 여부)
        base_score = 0.5 if (pdf_docs and web_docs) else 0.3
        
        # 상충 정보에 따른 감점
        conflict_penalty = len(conflicts) * 0.1
        
        # 합의 정보에 따른 가점
        consensus_bonus = min(len(consensus) * 0.1, 0.3)
        
        # 문서 품질 점수
        pdf_quality = np.mean([doc.get('final_score', doc.get('similarity', 0.5)) for doc in pdf_docs]) if pdf_docs else 0.5
        web_quality = np.mean([doc.get('final_score', doc.get('similarity', 0.5)) for doc in web_docs]) if web_docs else 0.5
        quality_score = (pdf_quality + web_quality) / 2
        
        final_score = base_score + consensus_bonus - conflict_penalty + (quality_score - 0.5) * 0.2
        return max(0.0, min(1.0, final_score))
    
    def _assess_reliability(self, pdf_docs: List[Dict], web_docs: List[Dict]) -> float:
        """정보 신뢰도 평가"""
        pdf_reliability = 0.8 if pdf_docs else 0.0  # PDF는 일반적으로 높은 신뢰도
        web_reliability = 0.6 if web_docs else 0.0  # 웹은 중간 신뢰도
        
        if pdf_docs and web_docs:
            return (pdf_reliability + web_reliability) / 2
        elif pdf_docs:
            return pdf_reliability
        elif web_docs:
            return web_reliability
        else:
            return 0.0
    
    def _get_web_text(self, doc: Dict) -> str:
        """웹 문서에서 텍스트 추출"""
        if hasattr(doc, 'text'):
            return doc.text
        return doc.get('text', '')
...
class ValidatorAgent:
    ...
    def validate_final_answer(self, query: str, answer: str, state: ResearchState, 
                            analysis_results: Dict) -> Dict[str, Any]:
        """최종 답변 검증"""
        validation_score = self._calculate_validation_score(query, answer, state, analysis_results)
        warnings = self._generate_warnings(state, analysis_results)
        
        return {
            "validation_score": validation_score,
            "warnings": warnings,
            "is_reliable": validation_score > 0.6,
            "recommendation": self._get_recommendation(validation_score, analysis_results)
        }
    
    def _calculate_validation_score(self, query: str, answer: str, state: ResearchState, 
                                  analysis_results: Dict) -> float:
        """검증 점수 계산"""
        # 정보 일관성 점수
        consistency_score = analysis_results.get('consistency', 0.5)
        
        # 신뢰도 점수
        reliability_score = analysis_results.get('reliability', 0.5)
        
        # 소스 다양성 점수
        pdf_count = analysis_results.get('pdf_docs_count', 0)
        web_count = analysis_results.get('web_docs_count', 0)
        diversity_score = min((pdf_count + web_count) / 5.0, 1.0)
        
        # 답변 완성도 점수 (길이 기반)
        completeness_score = min(len(answer) / 500.0, 1.0)
    ...
    def _generate_warnings(self, state: ResearchState, analysis_results: Dict) -> List[str]:
        """경고 메시지 생성"""
        warnings = []
        
        if analysis_results.get('consistency', 0) < 0.5:
            warnings.append("⚠️ 정보 일관성이 낮습니다. 상충되는 정보가 있을 수 있습니다.")
        
        if len(analysis_results.get('conflicts', [])) > 0:
            warnings.append(f"⚠️ {len(analysis_results['conflicts'])}개의 상충 정보가 발견되었습니다.")
        
        if analysis_results.get('pdf_docs_count', 0) == 0:
            warnings.append("⚠️ PDF 문서 정보가 없습니다. 웹 정보만으로 답변이 구성되었습니다.")
        
        if analysis_results.get('web_docs_count', 0) == 0:
            warnings.append("⚠️ 웹 정보가 없습니다. PDF 정보만으로 답변이 구성되었습니다.")
        
        if analysis_results.get('reliability', 0) < 0.4:
            warnings.append("⚠️ 전반적인 정보 신뢰도가 낮습니다.")
        
        return warnings
    
    def _get_recommendation(self, validation_score: float, analysis_results: Dict) -> str:
        """추천 사항 생성"""
        if validation_score > 0.8:
            return "✅ 높은 품질의 답변입니다. 신뢰할 수 있습니다."
        elif validation_score > 0.6:
            return "✅ 양호한 품질의 답변입니다. 일부 제한사항을 참고하세요."
        elif validation_score > 0.4:
            return "⚠️ 보통 품질의 답변입니다. 추가 검증을 권장합니다."
        else:
            return "❌ 낮은 품질의 답변입니다. 더 많은 정보가 필요합니다."
...
=== 출처 표시 요구사항 ===
1. 모든 정보에 대해 정확한 출처를 반드시 명시해야 합니다
...
class DeepResearchOrchestrator:
    ...
    def deep_research(self, query: str) -> Dict[str, Any]:
        ...
        max_cycles = 3 if complexity < 0.5 else 5
        ...
        for cycle in range(state.max_cycles):
            ...
            if cycle == 0:
                ...
                web_q = sum(1 for sq in search_queries if sq.search_web)
                research_log.append(f"생성된 검색 쿼리: {len(search_queries)}개 (웹검색: {web_q}개)")
            ...
            state.web_docs.extend([d for d in new_docs if d.get("source_type") == "WEB"])
        ...
        # Phase 3: 교차 검증
        st.info("🔍 PDF + 웹 문서 교차 검증 중...")
        state.phase = ResearchPhase.CROSS_VALIDATION
        analysis_results = self.analyzer.cross_validate_documents(query, state)
    
        # Phase 4: 종합 답변 생성
        st.info("📝 종합 답변 생성 중... (PDF + 웹 출처 포함)")
        state.phase = ResearchPhase.SYNTHESIS
        comprehensive_answer = self.synthesizer.synthesize_comprehensive_answer(query, state, analysis_results)
    
        # Phase 5: 최종 검증
        st.info("✅ 최종 답변 검증 중...")
        state.phase = ResearchPhase.FINAL_VALIDATION
        validation_results = self.validator.validate_final_answer(query, comprehensive_answer, state, analysis_results)
    
        research_log.append("종합 답변 생성 및 검증 완료")
        ...
        for doc in state.retrieved_docs[:15]:
            ...
            if src["source_type"] == "WEB":
                src.update({
                    ...
                    "web_domain": doc.get("web_domain", ""),
                    "web_crawl_time": doc.get("web_crawl_time", "")
                })
            sources.append(src)
...
min_chunk_length = st.sidebar.slider("최소 청크 길이", 30, 500, 50, help="청크가 너무 짧아지는 것을 방지하는 용도입니다")
max_chunk_length = st.sidebar.slider("최대 청크 길이", 200, 3000, 300, help="일정 수준까지는 최대 청크 크기에 성능이 비례합니다. 다만 최대 청크가 크면 VRAM 부족 오류가 발생할 수 있습니다.")
sentences_per_chunk = st.sidebar.slider("청크당 최대 문장 수", 1, 10, 2, help="청크가 너무 짧아지는 것을 방지하는 용도입니다")

# 웹크롤링 설정
st.sidebar.subheader("🌐 웹크롤링 설정")
enable_web_crawling = st.sidebar.checkbox("웹크롤링 활성화", value=WEB_CRAWLING_AVAILABLE, 
                                         help="활성화하면, 질문에 따라 자동으로 웹 검색 및 크롤링을 수행합니다")
...
# 시스템 초기화 버튼
if st.sidebar.button("🔄 Deep Research 준비", type="primary"):
...
if "research_bot" in st.session_state:
    ...
    query = st.text_input(
        "심층 연구할 주제를 입력하세요:",
        placeholder="예: 첫 번째 문서의 주된 내용을 요약해주세요.",
    ...
    if research_button and query:
        with st.spinner("🧠 다중 에이전트가 연구를 수행하고 있습니다..."):
    ...
    # 시스템이 초기화되지 않은 경우
    st.info("👆 왼쪽 사이드바에서 PDF 파일을 업로드하고 'Deep Research 준비' 버튼을 클릭해주세요.")
